...
1. First clone the branch `development`.
2. Go to the directory where you want to clone this repo.
3. Then type the command : ``git clone https://github.com/Target-Arc-Consulting/Prompt-Manager.git``  . This should clone the repo.
4. After cloning go to your terminal and then (cd into) go to the directory you have compose.yaml file.
5. Then type ``git checkout development``
6. Then type ``docker compose build`` . This should build your all images.
7. Then type ``docker compose up`` This should spin up all your containers up. Check with the container status in Docker Desktop in Containers section.
8. Go to your browser and type this ``http://localhost:3000`` in your search bar to make sure the app is rendered properly in your browser.
```
